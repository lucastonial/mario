...
//	spriteCacador->loadSprite(nomeArq.c_str(), 128, 82, 0, 46, 0, 53, 4, 2, 7);
	...
	spriteCacador->setPosition(0, 150);
...
// FunÁ„o de inicializaÁ„o da interface entre a Box2D e a ALLEGRO

// ===========================================

void InitBox2D(float scale, int alturaDaTela )
{
	scaleFactor = scale;  // Fisica/Allegro
	height = alturaDaTela; 
	// Define to vetor de gravidade
	b2Vec2 gravity(0.0f, -9.8f);
	// Permite que os objetos 'durmam'. Melhora o desempenho
	bool doSleep = true;
	// Inicializa a biblioteca Box2D
	world = new b2World(gravity, doSleep);
	// Define os par‚metro para a simulaÁ„o
	// Quanto maior, mais preciso, porÈm, mais lento
...
/*
	...
	*/
	cout << 
	Fisica->newBoxImage(WALL_ID,    //int id,
	                    s,				// CImage* sprite,
	                    1,						// float density,
	                    0.5,					// float friction,
	                    0.5,					// float restitution
	...
	//b2Vec2 pos(spriteCao->getX(), spriteCao->getY());
	//phys->setPosition(fisicaCao, pos);
